...
function buildSourceDirError(sourceDir: string): Record<string, unknown> {
  return {
    ...
    next_step: [
      ...
      '3. 重新执行：npx jiti src/scan-kb.ts scan --scope <scope> --source <dir> --root-name <name>',
    ],
    possible_causes: [
      '路径写错，或目录尚未创建',
...
function buildMissingPendingError(scope: string, pendingPath: string): Record<string, unknown> {
  return {
    ...
    next_step: [
      `1. 先执行：npx jiti src/scan-kb.ts scan --scope ${scope} --source <dir> --root-name <name>`,
      `2. 再执行：npx jiti src/scan-kb.ts scan --scope ${scope} --source <dir> --root-name <name> --results <ai-results.json>`,
    ],
    possible_causes: [
...
function handleVectorizeList(scope: string, scanIndexFile?: string): void {
  ...
  if (!scanIndex) {
    const command = scanIndexFile
      ? `npx jiti src/scan-kb.ts vectorize --scope ${scope} --scan-index ${path.resolve(scanIndexFile)}`
      : `npx jiti src/scan-kb.ts vectorize --scope ${scope}`;
...
function handleVectorizeComplete(scope: string, completeFile: string, scanIndexFile?: string): void {
  ...
  if (!scanIndex) {
    const command = scanIndexFile
      ? `npx jiti src/scan-kb.ts vectorize --scope ${scope} --scan-index ${path.resolve(scanIndexFile)} --complete ${completeFile}`
      : `npx jiti src/scan-kb.ts vectorize --scope ${scope} --complete ${completeFile}`;
    fail(buildMissingScanIndexError(scope, target, command));
  }
  ...
  .action(async (opts) => {
    try {
      ...
      try {
        ...
        const backupResult = autoBackup(config, scope, resultsFile, mode as 'full' | 'incremental');
        if (backupResult.ok) {
