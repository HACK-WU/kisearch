# SR-01 契约快照（`contract_base` = `freeze/REQ-20260924-001`）
...
> **2026-09-25（SR-01 窗口实跑后，由架构侧执行；属 L1 契约修订）**

| # | 修订 | 理由 |
|---|------|------|
| 1 | `chatDirFor(scope)` 的基准**必须来自 `loadConfig().dataDir`**，不得硬编码 `~/.ki` | 项目既有机制是 `getKbDir → loadConfig()`（支持 `KI_DATA_DIR` 覆盖）。硬编码会绕过 config 链路（用户改配置时 chat 不跟随）且让测试无法隔离。默认值下路径仍为 `~/.ki/chat/{scope}` |
| 2 | `acceptance-sr01.test.ts` 的 R23/R24 补**前置**（先建真实会话）与数据目录隔离 | R23/R24 是 store 级写操作（签名 `(scope, id, …)` → 走磁盘），原骨架传内存对象 → 必抛 `ConversationNotFound`。**判定标准（2 / 1 / 1）未变** |
| 3 | `verify/run.sh` 编译命令改用 `tsconfig.json` | `tsconfig.src.json` 的 include 只含 `src/zvec-engine/**`，对 `src/lib/chat/**` 覆盖为 0 → 编译检查假绿 |

> **流程偏离说明**：按 `delivery-slicing` 阶段 0 的两级修订约定，上表 1/2 属 L1 契约修订，
> **应在批次边界做**。本次因单窗口、无其他并发片，且阻塞发生在验收项上，故就地修正并如实记录。
> **判据未被放宽**（这是底线）—— 三处都是"让正确的检查真正生效"，不是"让红变绿"。
