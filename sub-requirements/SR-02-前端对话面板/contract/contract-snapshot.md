...
> ### ★ 推论（SR-02 提出，已采纳）：**测试参数本身是覆盖率的一部分**
>
> `CHUNK_SIZE` 的取值**不是随便定的**：
>
> ```text
> 若分块大小恰好等于（或整除）帧长 → readSseEvents 的【跨块缓冲分支永不执行】
>   → 即使跑了真实 HTTP，也测不出分块问题 → 【假绿】
> ```
>
> 因此 `test/chat/e2e-sr02-sources.test.ts` 的 **`CHUNK_SIZE = 7` 是刻意不对齐帧边界的**，
> **不得"优化"为整数**（如 1024 / 4096）—— 那样会**静默**失去对跨块缓冲的覆盖，
> 且测试仍然全绿（这正是最危险的地方）。
>
> **同类风险**：任何"为了让测试稳定"而调成整数值的测试参数，都应先确认它**没有同时把某个分支变成不可达**。
