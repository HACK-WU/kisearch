...
**边界（受 `scopeMode` 调制 · S-01 §3.5）**：`tags` 取值不在 DEFAULT_TAGS → 始终报错。`scope` 处置按 `scopeMode`：`default` 档缺省落 `default`、未注册 scope 自动创建（S-01 运行时化）；`strict` 档未传或未注册 scope → 返回 `{ ok:false, error:"unknown scope" }`，不静默落错。此举调和了原 S-01（自动创建）与本节（未声明报错）的矛盾——不再是二选一，而由 `scopeMode` 参数化（N13 的真实诉求）。
...
> **✅ N19（已收敛 · 2026-07-22，简化版）**：残留问题是**无状态 StreamableHTTP 下 daemon 拿不到 Agent 的 cwd，谁保证每次调用带对 scope**。**决策：不引入项目级绑定层（原 `X-Ki-Scope` / `X-Ki-Project` 方案已取消）**——沿用现有 scope 隔离即可，scope 本就是全局命名空间，隔离边界由「全局唯一的 scope 命名」保证，daemon 不做项目归属推断。护栏由 `scopeMode`（S-01 §3.5）提供：`default` 档单项目零摩擦（缺省落 default、自动创建）；`strict` 档要求显式传 scope 且必须在 `scopes` 白名单内，否则 fail-loud。
> - **诚实边界（已接受）**：`scopeMode` 拦不住「传了合法但错的 scope」（值合法、白名单内但非期望目标）；正确选择 scope 由调用方（CLI 参数 / LLM 工具参数）负责。此残留风险已知并接受。
> - 定级：🟡 → ✅（方案收敛为「scope 隔离 + `scopeMode` 护栏」，不含项目绑定层，落地随 S-01 `scopeMode`）。
