...
test('E2E-D1 full 导入：local KB 存原文（未清洗），向量 content 清洗后', { ...SKIP, timeout: 180_000 }, () => {
  const r = ki(['import', '--scope', SCOPE, '--source', ctx.sourceDir, '--group', ROOT_NAME]);
...
test('E2E-D3 --no-vector：local KB 原文写入、memoryIds 空', { ...SKIP, timeout: 180_000 }, () => {
  ...
  const r = ki(['import', '--scope', SCOPE, '--source', srcNoVec, '--group', ROOT_NAME, '--no-vector']);
...
test('E2E-D4 非 md 文件跳过 + 汇总提示', { ...SKIP, timeout: 180_000 }, () => {
  ...
  const r = ki(['import', '--scope', scopeD4, '--source', srcD4, '--group', ROOT_NAME]);
