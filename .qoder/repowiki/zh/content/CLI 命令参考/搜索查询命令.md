...
## 更新摘要
**变更内容**
- 更新了executeSearch函数性能优化部分，详细说明从N次标签查询到单次多标签OR查询的优化
- 增强了标签策略章节，说明新的单次查询机制
- 更新了性能与调优章节，包含新的优化效果
- 完善了架构总览图，反映新的查询流程
...
### 标签策略与多 Tag 行为
**已优化** 从 N 次标签查询优化为单次多标签 OR 查询

- 未指定 --tags 时，系统枚举当前 scope 下的所有 tag，使用单次查询进行 OR 组合检索
- 每次查询最多取 limit × tags.length 条结果，然后按内置优先级合并（ki-search > ki-relation > ki-path）
- 指定 --tags 时，直接按传入 tag 列表（逗号分隔）进行 OR 过滤检索
- **性能优化**：避免对同一查询重复执行 embedding 操作，显著减少延迟

**优化前**：每个 tag 单独查询，N 个 tag = N 次 embedding + N 次检索
**优化后**：单次查询包含所有 tag 的 OR 条件，仅需 1 次 embedding + 1 次检索

章节来源
- [src/search.ts:94-140](file://src/search.ts#L94-L140)
...
## 性能与调优
**已优化** executeSearch 函数性能显著提升

- 使用 --limit 控制返回条数，避免大结果集带来的传输与渲染开销
- 使用 --threshold 过滤低分命中，减少无关结果；推荐 0.02~0.03
- 使用 --tags 精确限定召回范围，降低检索面
- **性能优化**：executeSearch 函数从 N 次标签查询优化为单次多标签 OR 查询
  - 避免对同一查询重复执行 embedding 操作
  - 显著减少延迟，特别是在 tag 数量较多时
  - topk 按 tag 数放大保障每 tag 召回上限
...
- [src/lib/vector-client.ts:166-201](file://src/lib/vector-client.ts#L166-201)
...
- **性能问题**：如果查询仍然较慢，检查 tag 数量，考虑使用 --tags 精确过滤而非搜索全部标签
...
## 结论
ki search 提供基于"向量语义 + 全文 BM25 + RRF 融合"的混合检索能力，支持灵活的 scope/tag 过滤与可配置的阈值控制。通过 group/relation 定位与可选的原文召回，既能满足快速知识查找，也能支撑深入溯源。**最新的性能优化将标签查询从 N 次减少到单次 OR 查询，显著提升了检索效率**。合理设置 --limit、--threshold、--tags 可获得稳定且高效的检索体验。
