...
## 更新摘要
**所做更改**
- 增强了向量重建功能的批处理能力，支持200项批次处理
- 改进了rebuildScopeVectors函数的进度报告机制
- 添加了countScope函数的依赖注入以支持删除过程中的实时统计
- 实现了TTY和非TTY环境的动态进度显示
- 更新了向量重建流程以支持更好的用户体验和监控
...
## 简介
本文件面向"基于 zvec 的向量搜索引擎"实现，系统性说明嵌入模型配置与管理、索引构建与优化策略、向量维度管理、相似度计算、过滤条件编译、搜索路由机制、嵌入提供商扩展（如 SiliconFlow）、自定义嵌入集成、向量数据持久化，以及监控、故障恢复与性能调优指南。该引擎以 Node.js 进程内常驻方式封装 Rust 内核的 @zvec/zvec，提供高吞吐写入与低延迟检索能力，并通过 Worker 线程隔离原生句柄与锁资源。**最新更新**增强了向量重建功能的批处理能力，支持200项批次处理和改进的进度报告机制。
...
- 错误体系：errors.ts
- **新增**：向量重建模块：rebuild-vector.ts、progress.ts、vector-client.ts
...
J --> K["@zvec/zvec 原生库"]
B --> L["向量重建模块<br/>rebuild-vector.ts"]
L --> M["进度管理<br/>progress.ts"]
L --> N["向量客户端<br/>vector-client.ts"]
```
...
- [rebuild-vector.ts:329-519](file://src/lib/rebuild-vector.ts#L329-L519)
...
- **新增**：向量重建模块：支持200项批处理的向量重建功能，包含依赖注入的countScope函数和动态进度显示。
...
- **新增**：向量重建层提供批处理能力，支持200项批次处理和动态进度显示。
...
participant RB as "Rebuild Vector"
App->>Eng : upsert(docs)
...
- [rebuild-vector.ts:478-501](file://src/lib/rebuild-vector.ts#L478-L501)
...
### 增强向量重建功能
**更新** 向量重建功能现已支持200项批处理能力和改进的进度报告机制。

- **批处理能力**：
  - VECTORIZE_BATCH_SIZE = 200，与 import 链路 bulkVectorize 对齐
  - 分批提交避免单次 upsert 无中间态问题
  - docId 由 text+scope+tag 确定性生成，分批不改变幂等语义

- **依赖注入机制**：
  - countScope 函数通过依赖注入支持实时统计
  - CLI 路径注入真实 vectorCountScope 实现
  - 测试环境可注入 mock 实现避免触碰真实引擎

- **动态进度显示**：
  - TTY 环境：覆写当前行展示进度条（apt install 风格）
  - 非 TTY 环境：逐行输出，进度仍可读
  - 支持进度回调 onProgress(deleted) 用于删除过程

- **智能清理策略**：
  - 全量重建：先统计旧向量总数，再执行清理并显示进度
  - 局部重建：跳过清理，仅幂等覆盖匹配范围
  - 失败保护：清理失败中止，避免新旧混杂

```mermaid
flowchart TD
Start(["rebuildScopeVectors"]) --> Collect["收集四类条目<br/>内容 + relation + path + tag"]
Collect --> Partial{"局部重建?"}
Partial --> |是| NoDelete["跳过清理"]
Partial --> |否| Count["countScope 统计旧向量"]
Count --> Delete["deleteScope 清理<br/>onProgress 回调"]
NoDelete --> Batch["200项/批向量化"]
Delete --> Batch
Batch --> Progress["进度报告<br/>TTY/非TTY自适应"]
Progress --> Update["memoryId 回写"]
Update --> Done(["完成"])
```
...
- [vector-client.ts:759-781](file://src/lib/vector-client.ts#L759-L781)

**章节来源**
- [rebuild-vector.ts:329-519](file://src/lib/rebuild-vector.ts#L329-L519)
- [progress.ts:36-49](file://src/lib/progress.ts#L36-L49)
- [vector-client.ts:759-781](file://src/lib/vector-client.ts#L759-L781)
- [restore.ts:453-491](file://src/restore.ts#L453-L491)
...
- 模块耦合：
  ...
  - **新增**：rebuild-vector.ts 依赖 progress.ts、vector-client.ts、config.ts。
- 外部依赖：
...
Prox --> Work["worker.ts"]
Work --> Z["@zvec/zvec"]
RB["rebuild-vector.ts"] --> Prog["progress.ts"]
RB --> VC["vector-client.ts"]
RB --> CFG["config.ts"]
```
...
- [rebuild-vector.ts:23-33](file://src/lib/rebuild-vector.ts#L23-L33)
...
- [rebuild-vector.ts:35-36](file://src/lib/rebuild-vector.ts#L35-L36)
- [progress.ts:36-49](file://src/lib/progress.ts#L36-L49)
...
- **新增**：向量重建故障处理：
  - 清理失败：中止重建，避免新旧向量混杂
  - 零条目检测：局部重建范围内无条目时显式提示
  - 标签验证：防止内部保留标签被误用作自定义标签
...
- [rebuild-vector.ts:357-373](file://src/lib/rebuild-vector.ts#L357-L373)
- [rebuild-vector.ts:471-476](file://src/lib/rebuild-vector.ts#L471-L476)

## 结论
该向量引擎以稳定的门面 API 封装了 zvec 原生能力，通过嵌入层解耦、路由层灵活降级、过滤器编译保障安全、Worker 隔离确保稳定性，并提供完善的错误体系与探测恢复机制。**最新增强**的向量重建功能通过200项批处理、依赖注入的动态统计和TTY感知的进度显示，大幅提升了大规模向量重建的用户体验和可观测性。配合合理的批大小、去重、超时与重试策略，可在生产环境获得高吞吐写入与低延迟检索能力。
