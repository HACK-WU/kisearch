...
- [relation-map.ts:48-78](file://src/lib/relation-map.ts#L48-L78)
- [vector-client.ts:140-148](file://src/lib/vector-client.ts#L140-L148)
- [vector-client.ts:540-569](file://src/lib/vector-client.ts#L540-L569)
...
## 结论
本项目通过进程内 zvec 引擎、worker 串行化、向量客户端单例与空闲释放锁、relation-map TTL 缓存等机制，实现了高性能、高可用的语义检索能力。结合批量写入优化、分页限制与超时/重试策略，有效控制了内存与并发压力。建议在生产环境持续采集关键指标，结合基准测试与慢查询分析，动态调优参数以获得最佳性能。
